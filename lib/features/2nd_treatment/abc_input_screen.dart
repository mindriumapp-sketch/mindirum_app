...
import 'package:gad_app_team/models/get_address.dart';
...
class _AbcInputScreenState extends State<AbcInputScreen> {
  ...
  // MongoDB에 커스텀 칩 저장
  Future<void> _saveCustomChip(String type, String name) async {
    if (_uid == null || _uid!.isEmpty) return;
    try {
      await MongoService.instance.addCustomAbcChip(
        userId: _uid!,
        type: type,            // 'A' | 'B' | 'C-physical' | 'C-emotion' | 'C-behavior'
        name: name,
  ...
  //   }
  // }
  // 사용자 정의 칩 불러오기 함수 (MongoDB)
  Future<void> _loadCustomChips() async {
    if (_uid == null || _uid!.isEmpty) return;
    try {
      final docs = await MongoService.instance.fetchCustomAbcChips(userId: _uid!);
      for (final data in docs) {
        final String? type = data['type'] as String?;
        final String? label = data['name'] as String? ?? data['label'] as String?;
  ...
  void _addAKeyword() {
    final userName = context.read<MongoProvider>().userName;
    showDialog(
      context: context,
      builder:
          (_) => Dialog(
                    ...
                    if (_currentStep < 2) {
                      ...
                      if (_currentCSubStep < 2) {
                        ...
                        if (widget.isExampleMode) {
                          ...
                          if (!widget.isExampleMode) {
                            await _saveSelectedChipsToMongo();
                          }
      ...
      children: List.generate(_physicalChips.length, (i) {
        if (i == _physicalChips.length - 1) {
          ...
          return FilterChip(
            ...
            onDeleted:
                isCurrentSessionChip
                    ? () => _deleteCustomChip('CP', item.label, i)
                    : null,
            deleteIcon:
                isCurrentSessionChip
      ...
      children: List.generate(_emotionChips.length, (i) {
        if (i == _emotionChips.length - 1) {
          ...
          return FilterChip(
            ...
            onDeleted:
                isCurrentSessionChip
                    ? () => _deleteCustomChip('CE', item.label, i)
                    : null,
            deleteIcon:
                isCurrentSessionChip
                    ? const Icon(Icons.close, size: 18, color: Colors.redAccent)
      ...
      children: List.generate(_behaviorChips.length, (i) {
        if (i == _behaviorChips.length - 1) {
          ...
          return FilterChip(
            ...
            onDeleted:
                isCurrentSessionChip
                    ? () => _deleteCustomChip('CB', item.label, i)
                    : null,
            deleteIcon:
                isCurrentSessionChip
                    ? const Icon(Icons.close, size: 18, color: Colors.redAccent)
  ...
  // 3. Firestore 저장: "이번 세션에서 새로 추가했고, 실제로 선택하여 사용한 칩"만 저장
  // Future<void> _saveSelectedChipsToFirestore() async {
  //   final user = FirebaseAuth.instance.currentUser;
  //   if (user == null) return;

  //   // 중복 저장 방지용 - 혹시 동일 라벨이 이미 저장되어 있으면 건너뜀
  //   final snapshot = await _chipsRef(user.uid).get();
  //   final existing = snapshot.docs
  //       .map((doc) => {'type': doc['type'], 'label': doc['label']})
  //       .toSet();

  //   Future<void> maybeSave(String type, String label) async {
  //     // 이번 세션에서 사용자가 직접 추가했고(_isCurrentSessionChip), 실제로 선택까지 한 경우만 저장
  //     if (_isCurrentSessionChip(type, label) &&
  //         !existing.contains({'type': type, 'label': label})) {
  ...
  //     await maybeSave('A', _aGridChips[i].label);
  //   }

  //   // B칩
  //   for (final i in _selectedBGrid) {
  //     await maybeSave('B', _bGridChips[i].label);
  //   }

  //   // C-physical
  //   for (final i in _selectedPhysical) {
  //     await maybeSave('C-physical', _physicalChips[i].label);
  //   }

  //   // C-emotion
  //   for (final i in _selectedEmotion) {
  //     await maybeSave('C-emotion', _emotionChips[i].label);
  //   }

  //   // C-behavior
  //   for (final i in _selectedBehavior) {
  //     await maybeSave('C-behavior', _behaviorChips[i].label);
  //   }
  // }

  // 3. MongoDB 저장: "이번 세션에서 새로 추가했고, 실제로 선택하여 사용한 칩"만 저장
  Future<void> _saveSelectedChipsToMongo() async {
    if (_uid == null || _uid!.isEmpty) return;

    // 이미 저장된 {type,label} 조합 모으기(중복 방지)
    final existingDocs = await MongoService.instance.fetchCustomAbcChips(userId: _uid!);
    final existing = existingDocs
        .map((doc) => '${doc['type']}::${doc['name'] ?? doc['label']}')
        .toSet();

    Future<void> maybeSave(String type, String name) async {
      if (_isCurrentSessionChip(type, name) &&
          !existing.contains('$type::$name')) {
        await _saveCustomChip(type, name);
      }
    }
    ...
    // C-physical
    for (final i in _selectedPhysical) {
      await maybeSave('CP', _physicalChips[i].label);
    }
    // C-emotion
    for (final i in _selectedEmotion) {
      await maybeSave('CE', _emotionChips[i].label);
    }
    // C-behavior
    for (final i in _selectedBehavior) {
      await maybeSave('CB', _behaviorChips[i].label);
    }
  }

  // Firestore에서 커스텀 칩 삭제 함수
  Future<void> _deleteCustomChip(String type, String name, int index) async {
    if (_uid == null || _uid!.isEmpty) return;

    // 현재 세션에서 추가된 칩인 경우에만 MongoDB에서 삭제
    if (_isCurrentSessionChip(type, name)) {
      await MongoService.instance.deleteCustomAbcChip(
        userId: _uid!,
        type: type,
        name: name,
      );
    ...
    setState(() {
      ...
      _removeFromCurrentSession(type, name);
    });
  }
}
...
class AbcVisualizationScreenState extends State<AbcVisualizationScreen> {
  ...
  void _handleComplete() async {
    try {
      ...
      // 현재 위치 얻기 (가능한 경우)
      final addr = await getAddressKo();
      final Position? pos = addr.pos;
      final String? addressKo = addr.addressKo;

      // ABC 모델 데이터 (MongoDB 저장용)
      final data = <String, dynamic>{
        'activatingEvents'       : widget.activatingEventChips.map((e) => e.label).join(', '),
        'beliefs'                : widget.beliefChips.map((e) => e.label).join(', '),
        'consequenceP'  : widget.selectedPhysicalChips.join(', '),
        'consequenceE'   : widget.selectedEmotionChips.join(', '),
        'consequenceB'  : widget.selectedBehaviorChips.join(', '),
        'createdAt'             : DateTime.now().toUtc(),
        if (addressKo != null) 'addressName': addressKo,
        if (pos != null) 'latitude'  : pos.latitude,
        if (pos != null) 'longitude' : pos.longitude,
      };

      // MongoDB에 ABC 문서 저장 (삽입 후 abcId 반환)
      final String abcId = await MongoService.instance.insertAbcModel(
        userId: userId,
        data: data,
      );

      debugPrint('ABC 모델 저장 성공 - 사용자 ID: $userId, abcId: $abcId');
      if (!mounted) return;
          ...
          actions: [
            TextButton(
              ...
              onPressed: () async {
                ...
                await MongoService.instance.updateAbcModelGroup(
                  userId: userId,
                  abcId: abcId,
                  groupId: '1', // TODO: 기본 그룹 정책에 맞게
                );
                if (!mounted) return;
                final origin = widget.origin ?? ((ModalRoute.of(context)?.settings.arguments as Map?)?['origin'] as String?) ?? 'etc';
                final diary = (ModalRoute.of(context)?.settings.arguments as Map?)?['diary'];
                debugPrint('[abc_input] origin: $origin');
                if (origin == 'apply') {
                  // 적용하기 플로우: 알림 설정 건너뛰고 개입 선택으로 이동
                  int completed = 0;
                  try {
                    final Map<String, dynamic>? doc =
                        await MongoService.instance.fetchUser(userId: userId);
                    Map<String, dynamic>? userData;
                    final dynamic rawUser = doc?['user'];
                    if (rawUser is Map) {
                      userData = Map<String, dynamic>.from(rawUser);
                    }
                    final dynamic rawCompleted =
                        userData?['completedWeek'] ?? doc?['completedWeek'];
                    if (rawCompleted is int) {
                      completed = rawCompleted;
                    } else if (rawCompleted != null) {
                      completed = int.tryParse(rawCompleted.toString()) ?? 0;
                    }
                  } catch (e) {
                    debugPrint('completedWeek fetch failed: $e');
                  }
                  if (!mounted) return;
                  if (completed >= 4) {
                    Navigator.pushNamed(
