...
class _EducationPageState extends State<EducationPage> {
  ...
  Future<void> _incrementCompletedWeek() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final uid = prefs.getString('uid');
      if (uid == null || uid.isEmpty) return;
      final doc = await MongoService.instance.fetchUser(userId: uid);
      final current = (doc?['completedWeek'] as int?) ?? 0;
      await MongoService.instance.updateUserFields(
        userId: uid,
        updates: {
          'completedWeek': current + 1,
        },
      );
    } catch (_) {
      // 실패는 무시 (UX 유지)
